...
class AssignClassTeacherController extends Controller
{
    ...
    public function insert(Request $request)
    {
        if(!empty($request->teacher_id))
        {
            ...
            return redirect(url('admin/assign_class_teacher/list'))->with('success', "Assign Class to Teacher Successfully");
        }
        else
        {
    ...
    public function update(Request $request)
    {
        ...
        if(!empty($request->teacher_id))
        {
            foreach($request->teacher_id as $key => $teacher_id)
            {
                $getAlreadyFirst = AssignClassTeacherModel::getAlreadyFirst($request->class_id, $teacher_id);
                if(!empty($getAlreadyFirst))
                {
    ...
    public function update_single($id, Request $request)
    {
        ...
        if(!empty($getAlreadyFirst))
        {
            ...
            return redirect(url('admin/assign_class_teacher/list'))->with('success', "Status Successfully Update");
        }
        else
        {
            $save = AssignClassTeacherModel::getSingle($id);
